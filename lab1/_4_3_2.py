...
for noise_sigma in [0.09]:
    ...
    # with open(f"pickles/{noise_sigma}.pkl", "wb") as f:
...
#     with open(f"pickles/{noise_sigma}.pkl", "rb") as f:
#         _, val_perf_for_hidden = pickle.load(f)
#     mean = np.mean(val_perf_for_hidden, axis=1)
#     plt.plot(LAYERRANGE, mean, label=f"Noise sigma={noise_sigma}")
